...
class AdminDashboard {
    ...
    async loadContentData() {
        ...
        try {
            ...
            console.log('Fetching from: http://localhost:3000/api/content');
...
async function showContentModal(contentId = null) {
    ...
    try {
        ...
        if (contentId) {
            const response = await fetch(`http://localhost:3000/api/content?id=${contentId}`);
            const result = await response.json();
...
async function saveContent() {
    ...
    try {
        ...
        const url = currentEditingId ? 
            `http://localhost:3000/api/content/${currentEditingId}` : 
...
async function loadSubcategories() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/subcategories?category_id=${categoryId}`);
        const result = await response.json();
